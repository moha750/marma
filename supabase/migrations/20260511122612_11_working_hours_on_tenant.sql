...
BEGIN
  ...
  -- جلب ساعات عمل الملعب
  SELECT opening_time, closing_time INTO v_tenant
  ...
  v_day_start := (p_date::text || ' ' || v_tenant.opening_time::text)::timestamptz;
  IF v_tenant.closing_time <= v_tenant.opening_time THEN
    v_day_end := ((p_date + 1)::text || ' ' || v_tenant.closing_time::text)::timestamptz;
  ELSE
    v_day_end := (p_date::text || ' ' || v_tenant.closing_time::text)::timestamptz;
  END IF;
...
-- تحديث create_pending_booking لقراءة ساعات العمل من tenants
...
BEGIN
  ...
  -- التحقق أن start_time يطابق slot صحيح
  v_local_date := p_start_time::date;
  IF v_tenant.closing_time <= v_tenant.opening_time
     AND p_start_time::time < v_tenant.closing_time THEN
    v_local_date := v_local_date - 1;
  END IF;

  v_day_start := (v_local_date::text || ' ' || v_tenant.opening_time::text)::timestamptz;
