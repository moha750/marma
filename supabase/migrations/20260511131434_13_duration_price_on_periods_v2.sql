-- 1. إضافة المدة والسعر لكل فترة عمل
ALTER TABLE public.working_periods
  ...
  ADD COLUMN IF NOT EXISTS hourly_price numeric(10,2) NOT NULL DEFAULT 0;

-- 2. حذف المدة والسعر من الأرضيات
ALTER TABLE public.fields
  DROP COLUMN IF EXISTS slot_duration_minutes,
  DROP COLUMN IF EXISTS hourly_price;

-- 3. DROP get_available_slots (تغيير return type)
...
BEGIN
  ...
  FOR v_period IN
    SELECT open_time, close_time, slot_duration_minutes AS duration, hourly_price AS price
...
-- 4. تحديث create_pending_booking
...
-- 6. تحديث get_public_tenant_info: لا يرجع hourly_price
