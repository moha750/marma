-- جوّال المالك: يُلتقط في شاشة الإعداد (إلزامي) ويُخزَّن في profiles.
-- يوسّع create_owner_tenant ليأخذ p_phone مع تحقّق خفيف.
...
begin
  ...
  -- تحقّق خفيف: أرقام فقط بعد إزالة الرموز، 9 خانات على الأقل
  v_phone_digits := regexp_replace(v_phone, '[^0-9]', '', 'g');
  if length(v_phone_digits) < 9 then raise exception 'INVALID_PHONE' using errcode = 'P0001'; end if;
