-- get_available_slots: إضافة عمود is_booked (حجز حقيقي يغطّي الوقت، بصرف النظر عن مضيّ الوقت)
-- ليُحسب الإشغال في لوحة التحكم على المحجوز فعلاً لا على المواعيد المنقضية الفارغة.
-- is_available لم يتغيّر سلوكه إطلاقاً (= not is_past and not is_booked) فصفحة الحجز العامة سليمة.
...
create function public.get_available_slots(p_tenant_id uuid, p_field_id uuid, p_date date)
 returns table(slot_start timestamptz, slot_end timestamptz, is_available boolean, is_past boolean, is_booked boolean,
...
begin
  ...
  loop
    ...
    while v_slot_start < v_day_end loop
      ...
      -- محجوز فعلاً: يوجد حجز/حجب يغطّي هذا الوقت (بصرف النظر عن مضيّ الوقت)
      is_booked := exists (
      ...
      -- متاح للحجز: لم يمضِ وقته ولا هو محجوز — سلوك is_available لم يتغيّر
