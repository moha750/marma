-- التجربة المجانية: ٧ أيام → ٣٠ يومًا (شهر كامل).
-- تغيير واحد فقط داخل create_owner_tenant — بقية الدالة كما هي من
-- 20260817120000_trial_7_days.sql (وقبلها فرض صيغة الجوّال السعودي).
--
-- لماذا شهر: موسم الملاعب نفسه شهري — الدوري المحلي، الاشتراك الثابت
-- للفريق، فاتورة نهاية الشهر. أسبوعٌ لا يكفي ليمرّ المالك بدورة عمل
-- كاملة، فيحكم على النظام من أيام متفرّقة بدل أن يراه يُغلق شهرًا.
-- والتجربة أطول تعني أيضًا بيانات حقيقية في الحساب عند لحظة القرار، فلا
-- يبدأ الاشتراك من صفحة فارغة.
--
-- ملاحظات:
--   • يسري على الحسابات الجديدة فقط؛ التجارب الجارية تحتفظ بتاريخ انتهائها
--     كما هو (٧ أيام لمن بدأ قبل الهجرة). لتمديد الجارية: admin_extend_trial،
--     أو update مباشر على trial_ends_at.
...
--   • فرض صيغة الجوّال السعودي ^05[0-9]{8}$ يبقى كما هو.
--   • لا فترة سماح (20260818120000_remove_grace_period.sql) — القفل لحظة
--     انتهاء التجربة.
--   • TRIAL_DAYS في src/features/subscriptions/pricing.js يقابل هذا الرقم.
...
begin
  ...
  values (v_name, now() + interval '30 days', 'trial')
