...
>
> الحالة: تصميم مُعتمد (لم يُنفَّذ بعد). الإصدار 1.1 — 2026-07-30 — القرارات الخمسة محسومة (§16).
...
| 6. طلب **موافقة النشر** | Wallet Console → Publishing | قبلها: **Demo mode** — تُحفظ البطاقات على حسابات اختبار مُضافة يدوياً فقط. الموافقة تستغرق أيام عمل → **ابدأ بها في اليوم الأول** |
...
`PATCH` على الكائن → يظهر على جهاز العميل تلقائياً. لا APNs ولا مكافئ له. أرخص وأبسط جانب في المشروع.

> 🔬 **بند تحقّق مسبق (Spike B):** هل يحتاج **كل** Class جديد مراجعة من Google بعد اعتماد المُصدِر؟ إن نعم، فتفعيل الملعب سيتأخر أياماً — والبديل الجاهز: **Class واحد عام «مَرمى»** + هوية الملعب في `heroImage` و`textModulesData` على مستوى الـ Object (الشعار واللون على الـ Class فقط، فيصبح التخصيص أضعف لكن التفعيل فوري). اختبره على أول ملعبين.
...
| **3. المحافظ** ✅ | `wallet-apple` منشورة، وسيط Cloudflare، `card.html` + `functions/card.js`، RPCs عامة، زر واتساب في اللوحة. **أول بطاقة حقيقية صدرت وأُضيفت على iPhone (2026-07-31)**. جوجل معلّقة على موافقة المُصدِر | ✔ |
| **4. المزامنة اللحظية** ✅ | `wallet-sync` منشورة (APNs بمصادقة توكن ES256)، جسر `pg_net` مع خنق تدفّق بفهرس فريد جزئي، تنظيف الأجهزة الميتة عند 410، وcron تصريف كل ٥ دقائق. **Spike A محسوم: التوكن لا الشهادة.** جوجل تنتظر الاعتماد | ✔ |
...
| **7. الإطلاق** | موافقة نشر جوجل، ربط الخطط، إعلان داخل اللوحة، توثيق | 2 أيام |
...
| R3 | تأخّر/رفض موافقة جوجل | لا Android في الإطلاق | ابدأ الطلب يوم 1؛ Demo mode للتجربة؛ أطلق iOS أولاً |
| R4 | مراجعة كل Class (Spike B) | تفعيل الملعب يتأخر أياماً | بديل Class العام موصوف في §6.4 |
